...
const state = struct {
    ...
    var camera_pos: vec3 = .{ .x = 0.0, .y = 1.5, .z = 6.0 };
    var camera_front: vec3 = .{ .x = 0.0, .y = 0.0, .z = -1.0 };
    var camera_up: vec3 = vec3.up();
    var yaw: f32 = -90.0; // Initial rotation
    var pitch: f32 = 0.0;
};
...
export fn frame() void {
    ...
    // view-project matrix
    const view = mat4.lookat(state.camera_pos, vec3.zero(), vec3.up());
...
export fn input(event: ?*const sapp.Event) void {
    ...
    if (ev.type == .KEY_DOWN) {
        ...
        switch (ev.key_code) {
            .A => state.camera_pos.x += camera_speed,
            .D => state.camera_pos.x -= camera_speed,
            else => {},
        }
    }
}
